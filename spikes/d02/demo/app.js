...
async function enableSound(){try{audioContext??=new AudioContext();await audioContext.resume();sound=true;$('sound').textContent='Mute sound';}catch{status('Sound could not start. Click Enable sound to retry.',true)}}
...
async function load(file){if(!file)return;const request=++loadRequest;try{if(file.size>8*1024*1024)throw Error('Choose a .nes ROM smaller than 8 MiB.');const bytes=new Uint8Array(await file.arrayBuffer());validRom(bytes);
...
$('choose').onclick=$('change').onclick=()=>{$('file').value='';$('file').click()};$('file').onchange=()=>load($('file').files[0]);
...
$('pause').onclick=()=>{pause(!paused);screen.focus()};$('sound').onclick=()=>{if(sound){sound=false;flush();$('sound').textContent='Enable sound'}else enableSound();screen.focus()};
...
screen.addEventListener('keydown',e=>{if(keyMap[e.code]){e.preventDefault();keys|=keyMap[e.code]}});window.addEventListener('keyup',e=>{if(keyMap[e.code])keys&=~keyMap[e.code]});
