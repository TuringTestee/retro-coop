...
''')
        ...
        try:
            with self.log.open('w') as log:
                self.process=subprocess.Popen([self.executable,'-v','-c',str(config)],stdout=log,stderr=subprocess.STDOUT)
    ...
    def environment(self):
        return {'TURN_URLS':f'turn:127.0.0.1:{self.port}?transport=udp','TURN_SECRET':self.secret,'TURN_ROOM_LIMIT':'1'}

    def error_codes(self):
        # Only numeric codes leave the private temporary directory.
        return dict(collections.Counter(re.findall(r'error (\d{3})',self.log.read_text(errors='replace'))))
