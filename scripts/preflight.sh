...
python3 -c 'import ast, pathlib; root=pathlib.Path("spikes/d02"); [ast.parse(p.read_text()) for p in [*root.glob("*.py"), *(root/"demo").glob("*.py"), *pathlib.Path("scripts/foundation").glob("*.py"), *pathlib.Path("scripts/rooms").glob("*.py")]]'
