...
async def main():
    ...
    try:
        ...
        async with async_playwright() as playwright:
            ...
            release.set()
            await host.get_by_role('button',name='Retry upload').wait_for()
            ...
            await host.get_by_role('button',name='Retry upload').click()
            await host.wait_for_function("document.body.textContent.includes('Upload connection failed')",timeout=15000)
            await host.get_by_role('button',name='Retry upload').wait_for()
            failure_text=await host.get_by_role('button',name='Retry upload').locator('..').inner_text()
            assert 'Retry upload' in failure_text and ('failed' in failure_text.lower() or 'timed out' in failure_text.lower()),failure_text
            assert await host.get_by_test_id('room-view').count()==0
            ...
            await invalid.locator('.selection-status').wait_for()
