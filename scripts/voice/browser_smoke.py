...
parser=argparse.ArgumentParser();parser.add_argument('--chrome',action='store_true');parser.add_argument('--pair',choices=['Chrome-Chrome','Chrome-Firefox','Firefox-Firefox'],default='Chrome-Chrome');parser.add_argument('--output',default='voice.local.json');parser.add_argument('--relay',action='store_true');parser.add_argument('--turnserver',default='turnserver');args=parser.parse_args();started=time.monotonic()
root=Path(__file__).resolve().parents[2];sys.path.insert(0,str(root/'scripts/peer'));from fixture import LocalTurn
fixture=LocalTurn(args.turnserver) if args.relay else contextlib.nullcontext();stack=contextlib.ExitStack();server=None;pages=[]
try:
 turn=stack.enter_context(fixture)
 server=subprocess.Popen(['node','scripts/rooms/browser-server.ts'],cwd=root,env={**os.environ,**(turn.environment() if turn else {'TURN_URLS':'','TURN_SECRET':''})},stdout=subprocess.PIPE,text=True)
 url=json.loads(server.stdout.readline())['url'];rom=(root/'apps/client/dist/generated/diagnostic.nes').read_bytes()
 p=sync_playwright().start();stack.callback(p.stop)
 browsers={}
 for kind in set(args.pair.split('-')):
  browsers[kind]=p.chromium.launch(**({'channel':'chrome'} if args.chrome else {}),ignore_default_args=['--mute-audio'],args=['--use-fake-device-for-media-stream','--use-fake-ui-for-media-stream']) if kind=='Chrome' else p.firefox.launch(firefox_user_prefs={'media.navigator.streams.fake':True,'media.navigator.permission.disabled':True})
 errors=[];kinds=iter(args.pair.split('-'))
 def page(url):
  tab=browsers[next(kinds)].new_page();pages.append(tab);tab.on('pageerror',lambda e:errors.append(str(e)));tab.add_init_script((root/'scripts/peer/diagnostics.js').read_text()+'''window.candidateShapes=[];const RawSocket=WebSocket;window.WebSocket=class extends RawSocket{send(raw){const v=JSON.parse(raw);if(v.type==='peerSignal'&&v.signal.kind==='candidate'){const c=v.signal.candidate;candidateShapes.push({keys:Object.keys(c),empty:c.candidate==='',prefix:c.candidate?.startsWith('candidate:'),mid:c.sdpMid,index:c.sdpMLineIndex})}return super.send(raw)}};window.peerErrors=[];for(const name of ['setRemoteDescription','setLocalDescription','addIceCandidate','createAnswer','getStats']){const original=RTCPeerConnection.prototype[name];RTCPeerConnection.prototype[name]=async function(...args){try{return await original.apply(this,args)}catch(error){peerErrors.push({operation:name,name:error.name});throw error}}}window.pcs=[];window.captures=[];const replace=RTCRtpSender.prototype.replaceTrack;RTCRtpSender.prototype.replaceTrack=function(track){if(window.rejectAttachment&&track)return Promise.reject(new DOMException('fixture attach failure','InvalidModificationError'));return replace.call(this,track)};window.voiceAudio=[];const NativeAudio=Audio;window.Audio=class extends NativeAudio{constructor(...args){super(...args);voiceAudio.push(this)}play(){if(window.blockPlayback)return Promise.reject(Error('fixture playback denial'));return super.play()}};const Native=RTCPeerConnection;window.RTCPeerConnection=class extends Native{constructor(...args){super(...args);pcs.push(this)}};const capture=navigator.mediaDevices.getUserMedia.bind(navigator.mediaDevices);navigator.mediaDevices.getUserMedia=async c=>{if(window.denyCapture)throw new DOMException('fixture denial','NotAllowedError');if(window.missingDevice)throw new DOMException('fixture device missing','NotFoundError');if(window.holdCapture)await new Promise(resolve=>window.releaseCapture=resolve);const s=await capture(c);captures.push(s);return s};''');tab.goto(url);return tab
 host=page(url);host.locator('.room-panel').get_by_label('Connection privacy',exact=True).select_option('relay' if args.relay else 'standard');host.set_input_files('input[type=file]',{'name':'fixture.nes','mimeType':'application/octet-stream','buffer':rom});host.get_by_test_id('room-view').wait_for()
 ...
 for tab in [host,guest]:
  tab.wait_for_function("route=>document.querySelector('[data-testid=connection-status]').textContent.includes('Route: '+route)",arg='relay' if args.relay else 'direct')
  if args.relay:assert tab.evaluate("async()=>{const stats=await pcs.at(-1).getStats();return [...stats.values()].some(s=>s.type==='transport'&&s.selectedCandidatePairId&&stats.get(stats.get(s.selectedCandidatePairId).localCandidateId)?.candidateType==='relay')}")
 ...
 for tab in pages:
  try:
   failure.append(tab.evaluate("""async()=>({diagnostics:peerDiagnostics(),peerErrors,candidateShapes,focus:document.hasFocus(),hidden:document.hidden,microphone:document.querySelector('[data-testid=microphone-status]')?.textContent,connection:document.querySelector('[data-testid=connection-status]')?.textContent,tracks:captures.map(s=>s.getTracks().map(t=>({state:t.readyState,enabled:t.enabled}))),peers:await Promise.all(pcs.map(async pc=>({state:pc.connectionState,stats:[...(await pc.getStats().catch(()=>new Map())).values()].filter(s=>['transport','inbound-rtp','outbound-rtp'].includes(s.type)).map(s=>({type:s.type,kind:s.kind,dtls:s.dtlsState,packetsReceived:s.packetsReceived,packetsSent:s.packetsSent,totalAudioEnergy:s.totalAudioEnergy}))})))})"""))
  except Exception as error:failure.append({'diagnostics_error':str(error)[:500]})
 Path(args.output).write_text(json.dumps({'failure':failure,'page_errors':errors},indent=2)+'\n');print(json.dumps({'failure':failure,'page_errors':errors}),flush=True)
 raise
finally:
