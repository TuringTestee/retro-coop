...
def verify_persistence(browser,url,rom,worker_path,output):
    ...
    page.get_by_role('button',name='Settings',exact=True).click()
    settings=page.get_by_role('dialog',name='Local settings')
    ...
    page.get_by_role('button',name='Close saves',exact=True).click()
    ...
    panel.get_by_role('button',name='Close local data',exact=True).click();settings.get_by_role('button',name='Done',exact=True).click()
