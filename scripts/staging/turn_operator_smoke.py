...
''')
    ...
    provision = ["sh", str(root / "scripts/staging/provision_turn.sh"), "test-project", "us-central1-a",
    ...
    subprocess.run(["sh", str(root / "scripts/staging/teardown.sh"), "test-project"], cwd=root, env=environment, check=True, capture_output=True)
