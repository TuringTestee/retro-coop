...
|---|---|---|---|
| J1: Be first in an included-game room | Open the public list; see Super Tilt Bro or From Below at **0/2 · Waiting for host**. | Select **Join**. The service atomically assigns this visitor Host/P1; the browser downloads and verifies the included file. | Enter the same room at 1/2 with **Start game**, **Invite**, and **Leave**. If another visitor claimed first, show the new room state and offer Guest join or a fresh 0/2 room. If the asset fails, keep or release the claim honestly and offer Retry/Leave. |
| J2: Join an existing host | Open the same list; identify a specific room by game, host, code, occupancy, and state. | Select **Join** on a 1/2 room. Reserve P2 and prepare the exact game locally. | Enter that room as Guest/P2; see the host and whether the game is waiting or playing. Correct an unavailable/mismatched file, retry a failed connection, or leave without changing the host's game. |
| J3: Start and play as host | From a 1/2 room, decide whether to wait or play now. | Select **Start game**. Show loading and the actual start result; do not imply a second player exists. | Play solo in the same room. The Player 2 slot remains visibly open where a safe later join works. If late join is unavailable in the current build, show why the playing room cannot yet accept a guest. Pause, retry, or leave on failure. |
| J4: Add a guest to ongoing play | Find a playing room with an open, joinable P2 slot; the host may also share its invite. | Guest selects **Join**; both peers match file and build. The host sees a join request while continuing play until a safe pause. | Transfer validated progress and resume together after both acknowledge. A mismatch, decline, timeout, or failed transfer keeps the host's timeline intact and offers retry or continued solo play. |
| J5: Host a personal NES file | Open **Host a game**; choose a local `.nes` file, Public/Unlisted, and a room label. | Validate locally, then create a room. Show creation progress and the confirmed public row or invite. | Other browsers see a public room in the ordinary list and can join with their own exact matching file. Never publish a local path or file bytes. Invalid file, capacity, cancellation, or stale creation offers correction without a ghost room. |
| J6: Find a friend's room | Search by room/host or exact public code; an unlisted room arrives by invitation. | Open the matching row or invite preview; inspect game, host, slots, and status, then Join if available. | Join that exact session. Duplicated labels stay separate by code; full, closed, stale, or unlisted search results explain the limit and offer a valid alternative. |
...
| Waiting or playing 1/2 row (J2/J4) | Join, or Join ongoing game when supported | Current host, game, open slot, state, matching-file requirement | Guest preparation, late join, or unavailable reason |
| Local-file host form (J5) | Create room | Selected file validation, public label, access, guest file need | Confirmed row or retained selection with correction |
