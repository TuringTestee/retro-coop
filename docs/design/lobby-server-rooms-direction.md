...
- The first person to join an empty included-game room becomes **host and Player 1**. They may start at any time, including alone. The second person joins as Player 2 when the room can accept them. For From Below, the second person shares or takes turns with the single game controller; the UI must say so.
...
Host Start while alone keeps that room's identity and open Player 2 slot. Joining after gameplay has progressed needs the already planned progress-preserving late-join path; until that path works, the UI must show the playing room as temporarily unavailable for Join. A host leaving closes their room; the service-created room supply is a separate concern and remains available. A service restart recreates empty included-game rooms and reports interrupted human sessions honestly.
