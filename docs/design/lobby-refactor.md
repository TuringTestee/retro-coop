...
- The host decides when to start. Starting alone keeps the same room and its open guest slot. A guest can join before Start or join ongoing play after the progress-preserving late-join path succeeds. A playing room must not advertise Join if that path is unavailable.
...
- **Host your NES file** is the one path for creating a player-owned room. The host chooses a local file, public label, and Public or Unlisted access. A public room appears in other browsers' directory; an unlisted room joins through its invitation. Local file bytes, filesystem path, and filename are never public room metadata. A guest supplies an exact matching file unless the verified included asset can be downloaded.
...
2. Give every visible feature a discoverable control, usable state, truthful feedback, and failure or exit path. The [feature check](lobby-wireframe-v2.md#feature-to-journey-ui-check) maps those surfaces.
3. Show information when the next decision needs it: host role before claiming 0/2, file requirements before joining a player-owned room, Start's effect in the host room, and timeline choices at late join.
...
The [current page-by-page ASCII wireframe](lobby-wireframe-v2.md) shows directory, local-file hosting, host/guest room, play, and in-place recovery. Its [first iteration](lobby-wireframe-v1.md) and [page critique](lobby-critique-v1.md) show why labels and states changed; the [earlier lobby proposal](lobby-browser-proposal.md) remains a previous iteration. The five most recent iterations are retained; superseded older drafts can be removed without deleting an approved baseline or source evidence.
