...
This document governs the affected discovery and lobby behavior over conflicting passages in [the platform design](browser-nes-platform.md), [included-game design](included-games.md), and [earlier UI sketches](browser-nes-ui.md). Their unaffected technical and release requirements remain in force. The [delivery amendment](../implementation/lobby-refactor.md) states the work and current verification gates.

Specifically, host Start replaces AC-03's automatic-start clause and AC-13's no-Start-gate clause. Progress-preserving later join and its AC-13 timing measurement are deferred; a solo-started room visibly rejects Join. The existing fast-start, voice, accessibility, and recovery evidence requirements continue under the new integrated gate #66. Closed [D21/#25](https://github.com/TuringTestee/retro-coop/issues/25) does not accept these changed journeys.
