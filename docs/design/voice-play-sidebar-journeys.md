...
| V2 | Speaking player sees `Mic on` → chooses `Mute` → card shows `Muted` → chooses `Unmute` when ready. Push-to-talk player holds the displayed mapped key or `Hold to talk` control; release stops transmission. | On blur or reconnect, mic remains muted/off. Re-enable deliberately; leave stops tracks. |
...
V1/V2/V3 derive from the user request plus approved U7 voice behavior. C1/C2/C3 derive from the new shortcut request and current remappable controls. E1 preserves the approved release path. The primary route for editing remains Settings; the side card is a readout, not a second editor.
