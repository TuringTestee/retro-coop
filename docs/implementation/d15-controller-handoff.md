...
The harness now arms scripted controls while paused and measures the full 30/600-second qualification interval after resume. Input cadence, transition counts, all acceptance thresholds, the 750-second network timeout and the 30-minute CI deadline are unchanged. The existing eight-second fault/journey smoke still measures both epochs, preserving its 180-second suite allocation. A diagnostic `--initial-manual-frames 508` option exercises the slow setup without changing the default workload. No product source or emulator code changed.
