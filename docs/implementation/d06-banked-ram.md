...
This is a proposal and unexecuted fixture, not passing hardware evidence. The independent D11 timing matrix currently owns the quiet native/browser window. PR #58 remains unchanged and separately reviewed.
...
Proposed reproducible delivery, after the before-failure result:
...
Vendoring adds maintained source and provenance overhead, but avoids creating an externally hosted fork or making clean builds depend on an unversioned patch cache. No dependency/runtime source has been changed yet.
