...
Record actual command duration, exact PR head, independent review and CI run link in the PR's current evidence comment. Current pre-flight and CI budgets follow the governing [verification strategy](../browser-nes-platform.md#verification-strategy); retries cannot extend the original workflow deadline.
