...
The committed evidence folder records 40 passing Node tests and the timed complete preflight. Native browser/coordinator proof sends before the guest loads a ROM, rejects pre-join history, renders hostile markup literally, releases a held game button while typing, disables oversized sends, preserves rate-limited text with countdown/retry, retains an unsent draft through socket loss and reconnect without automatic delivery, retries a delivered message after both its event and acknowledgement were suppressed without broadcasting a duplicate, and clears messages on leave/rejoin. Existing room and foundation/settings/battery browser probes remain regression checks. Chrome default global mute is removed; game gain remains muted.

The first browser run exposed a textarea's wrapped label acquiring its text content after typing. The field now uses an explicit `htmlFor`/`id` label; the same typing/oversize/retry workload passes. Visual inspection also caught a stale general room cooldown after chat recovery; chat-specific retry state now owns that countdown. No tests were relaxed to hide those failures. Combined directory/peer integration found a lost-ack harness race: the suppression hook could start before the previous sender acknowledgement arrived. It now waits for that prior delivery explicitly. One five-browser-suite concurrent author run then timed out waiting for post-reconnect delivery without preserving its error detail; the cause is unclassified. The serial rerun, a concurrent chat/foundation rerun and final concurrent chat/peer run all pass; the permanent probe now captures only error codes, command kinds and delivery status on failure. This remains an honest development failure, not a claimed before/fix proof. The CI runs these browser probes sequentially.

Integration includes the D09 directory and D10 peer candidate (including timeout publication and whitespace repair). Chat remains usable when Relay only is selected but the local relay is unavailable; the browser checks this directly. D09 is merged; D10 acceptance/merge is a delivery gate for this composed branch. No peer policy or protocol validator was forked. The explicit current-head preflight runs after committing source and evidence, because a raw failure log in the D10 candidate exposed trailing spaces after its earlier author checks.
