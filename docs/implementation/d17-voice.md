...
The first [CI run](https://github.com/TuringTestee/retro-coop/actions/runs/35500884475) failed the default headless-shell microphone request; its three network cells passed. The [minimal capture comparison](evidence/d17/headless-capture-results.txt) and [actual before failure](evidence/d17/headless-shell-before.json) preserve that result. After selecting full Chromium, all six unchanged local workloads passed with the exact CI browser versions (3.80/4.39s Chrome–Chrome, 9.40/10.87s mixed, 9.00/8.44s Firefox–Firefox). The manifest links each result; new CI remains required.

After persistence PR #55 merged, source `4cabff2` preserves both voice and Local data Settings controls. The complete [integrated foundation/settings/persistence probe](evidence/d17/integrated-foundation.json) passed in 60.99s; all six integrated voice cells also passed (4.20/9.42s Chrome–Chrome, 10.96/15.28s mixed, 7.82/7.52s Firefox–Firefox). These browser workloads are separate from the under-60s generic pre-flight. The manifest pins the merged base and each result; final-head CI and independent reassessment remain pending.
