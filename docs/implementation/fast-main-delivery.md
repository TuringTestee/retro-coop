...
The user requested on 2026-09-25: “the release gate should be just 30s long, 10 min is too long”; “setup CICD”; “CI should be short, and CD should be also under 5min”; “add post release testing” with rollback when needed; and “keep main CD synced with https://retro-coop.atobot.cloud/”. After the custom CD workflow failed, the user directed us to remove it and use `einaregilsson/beanstalk-deploy@v21`, following [their backend workflow](https://github.com/covariant-cloud/covariant-backend/tree/master/.github/workflows) and [frontend workflow](https://github.com/covariant-cloud/page1001/blob/master/.github/workflows/aws_fe_cd.yml). Both examples package the application and call that action with the package path and EB application, environment, region and version. Retro Coop additionally passes its temporary OIDC session token and checked image bundle. The user then said, “repurpuse the planning PR, just do it, use what I told you, do the work in that PR”; PR #154 therefore contains this amendment and its implementation together. Environment-specific account IDs and ARNs must come from GitHub configuration or AWS at runtime, not tracked workflow or release code. The [verification strategy](browser-nes-platform.md#verification-strategy) owns the test policy.
...
|---|---|---|
| Fast 30-second main release gate (`PVTI_lADOE0HVec4BkDjlzg8znF4`) | Reviewed delivery plan | Root: measured browser-pair evidence and CI timing |
