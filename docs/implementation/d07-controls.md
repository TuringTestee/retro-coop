...
Screenshots use `foundation.local.settings-before.png`, `foundation.local.settings-after.png` and `foundation.local.settings-mobile.png`, covered by the existing core artifact's `foundation.local.*.png` upload. Required CI still uses the shared 30-minute deadline; no new long post-submit suite is introduced.
