...
The focused regression is a separate under-60s CI step inside the existing shared30-minute deadline. The generic `timeout 60s sh scripts/preflight.sh` remains separate. Existing foundation/storage tests must pass with the new asset path; their scope is unchanged. Exact candidate/proof revisions and measured results are recorded with the PR. Root remains the sole merge owner after fresh independent acceptance and required CI.
