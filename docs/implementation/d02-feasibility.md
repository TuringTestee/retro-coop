...
| Final worker and complete-codec browser run | Pending in this draft. Earlier results above use a main-thread probe/trusted local raw restore; they do not substitute for the final worker+codec run. |
...
Prepare tools and run the README preflight. No user ROM is needed for the regression suite:
