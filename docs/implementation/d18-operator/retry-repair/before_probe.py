...
with tempfile.TemporaryDirectory(prefix='retro-operator-browser-') as directory:
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            fresh = page(url)
            fresh.get_by_role('button', name='Retry directory', exact=True).wait_for()
            fresh.screenshot(path=str(output.with_suffix('.retry.png')), full_page=True)
            print(json.dumps({'observed_room_status':fresh.get_by_test_id('room-status').inner_text(), 'observed_directory_status':fresh.locator('.directory-panel [role=status]').inner_text()}),flush=True)
            assert 'Access is temporarily restricted' in fresh.get_by_test_id('room-status').inner_text()
            fresh.reload()
