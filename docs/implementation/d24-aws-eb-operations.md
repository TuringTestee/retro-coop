...
The push workflow for the exact merged main commit saves four native ARM64 Docker image tarballs plus SHA-256 checksums as the `aws-eb-arm64-images` artifact. After that CI run succeeds, `.github/workflows/deploy.yml` checks that its push SHA is still the current main head, assumes the main-scoped AWS OIDC role, and runs `build_release.py --ci-run-id` against that exact run. It verifies the checksums and `linux/arm64` images, publishes immutable ECR digests, and updates the existing EB environment with the source revision and asset hashes. The deploy script checks Ready/Green, the one-instance boundary and public HTTPS health before CD reports success. The workflow records the prior and new EB versions and separate bundle/update durations. A failed CI run or superseded main commit leaves the current website in place. The later public two-browser acceptance and rollback workflow is tracked separately in [fast main delivery](fast-main-delivery.md).

`deploy/aws-eb/github-cd-role.yaml` owns the GitHub OIDC provider and dedicated `retro-coop-main-cd` role. Its trust accepts only this repository's main branch with audience `sts.amazonaws.com`; the template owns the exact subject value. Its policy scopes writes to this website's ECR repository, release prefix, EB application/environment, DNS A record and expected-IP parameter. Provision or update it with `aws cloudformation deploy --template-file deploy/aws-eb/github-cd-role.yaml --stack-name retro-coop-github-main-cd --capabilities CAPABILITY_NAMED_IAM --region us-east-1`. The workflow uses short-lived OIDC credentials; GitHub stores no AWS key. The local commands below remain an operator recovery path.

The job's 150-minute ceiling is a failure-recovery allowance, not a target runtime. The OIDC role allows and the workflow requests a three-hour AWS session so credentials remain valid through that ceiling and final reporting. The existing EB operator can spend 30 minutes each on a settings update, version update, version restoration and settings restoration, plus five minutes observing a failed update; packaging and live checks need additional time. On the normal unchanged-settings path, a 30-minute EB wait does not abandon a still-running update: the operator observes it for another bounded 30 minutes and accepts only a verified new Ready/Green version or reports the stable old version. A release that cannot reach a stable state reports its observed EB status for operator repair; it does not claim the site matches main.
