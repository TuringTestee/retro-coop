...
`deploy/aws-eb/github-cd-role.yaml` owns the GitHub OIDC provider and dedicated `retro-coop-main-cd` role. Its trust accepts only `repo:TuringTestee/retro-coop:ref:refs/heads/main` with audience `sts.amazonaws.com`; its policy scopes writes to this website's ECR repository, release prefix, EB application/environment, DNS A record and expected-IP parameter. Provision or update it with `aws cloudformation deploy --template-file deploy/aws-eb/github-cd-role.yaml --stack-name retro-coop-github-main-cd --capabilities CAPABILITY_NAMED_IAM --region us-east-1`. The workflow uses short-lived OIDC credentials; GitHub stores no AWS key. The local commands below remain an operator recovery path.
