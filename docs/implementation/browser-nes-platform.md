...
Continue evolving pre-flight with focused fast type/schema checks and deterministic short fixtures, keeping the entire command below 60 seconds. Pull-request CI enforces one five-minute wall-clock deadline across build, the relevant browser journey, the final gate and retries. Current `main` CI runs the full core/network qualification under a separate 30-minute deadline. The accepted [fast main delivery plan](fast-main-delivery.md) replaces that release gate with 30 seconds of gameplay per browser pair and a short main pipeline when implemented; until then, the existing workflow and deadline remain in force. The full qualification moves to a scheduled/manual post-release workflow with its own owner and evidence, and stops blocking CD. Keep CI dependencies pinned and document commands in the README.

Presubmit coverage must include fast deterministic checks for canonical serialization and malformed input, matching/mismatching fingerprints, atomic joins and authorization, privacy of outbound traffic, coordinated epoch changes, and local save compatibility, plus a real browser journey relevant to the change. The ten-minute network session matrix currently runs after merge on `main` and blocks release. Under the [fast main delivery plan](fast-main-delivery.md), a 30-second measured gameplay interval per browser pair becomes the release check; keep critical synchronization, controller, pause and page-error assertions. Run the ten-minute matrix after release on a schedule and on demand, retain its actual full-run time and evidence, and investigate failures. Roll back a confirmed live regression; infrastructure-only failures remain visible without undoing a healthy release. Test forced TURN and direct connection in staging with credentials scoped to the run.
...
Essential pull-request evidence uses focused checks for the changed journey. The complete core/network matrix currently runs after merge on `main`; after the fast main delivery work is integrated, it moves to scheduled/manual post-release qualification. The 30-minute-per-route AC-16 sessions, full per-hardware qualification and repeated startup samples remain separately timed staging validation jobs with named developer/operator ownership. Report release-validation results separately from pull-request CI and post-submit qualification.
