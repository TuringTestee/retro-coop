...
Presubmit coverage must include fast deterministic checks for canonical serialization and malformed input, matching/mismatching fingerprints, atomic joins and authorization, privacy of outbound traffic, coordinated epoch changes, and local save compatibility, plus a real browser journey relevant to the change. The ten-minute network session matrix and full core suite no longer block main delivery. The existing `spikes/d02/ci_job.sh core` and `network-full` commands remain runnable for the dedicated scheduled/manual post-release workflow; that workflow's card owns their schedule and publication. Its results must retain actual full-run time and evidence. Investigate failures and roll back a confirmed live regression; infrastructure-only failures remain visible without undoing a healthy release. The release gate tests forced TURN and direct connection with credentials scoped to the run.
