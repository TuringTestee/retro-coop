...
Essential pull-request evidence uses focused checks for the changed journey. The complete core/network matrix runs after merge on `main`; the 30-minute-per-route AC-16 sessions, full per-hardware qualification and repeated startup samples remain separately timed staging release-validation jobs with named developer/operator ownership. Report release-validation results separately from pull-request CI and post-submit qualification.
