...
Use [the design and AC-01 through AC-16](../../docs/design/browser-nes-platform.md) as the scope reference. The original delivery plan and [two-game catalog amendment](included-games.md) were approved and merged; live status, ownership and dependencies reside in [epic #2](https://github.com/TuringTestee/retro-coop/issues/2), its child issues and GitHub Project. The catalog amendment retains exact asset/content ownership. Checks and local review must identify the actual base/head under review.
The approved [room game transfer plan](room-rom-transfer.md) and its [human behavior](../design/room-rom-transfer.md) supersede the original no-upload/guest-matching-file architecture for custom rooms; issues #87–#91 are integrated. PR #101 approved CG1/CG2 in the [Create Game plan](create-game-library.md), changing the immediate-host entry after implementation; CG3/CG4 in that file are pending PR #103 approval. All other delivery and verification rules continue to apply.
The proposed [lobby refactor plan](lobby-refactor.md) owns the changed first-join, manual-Start, and single-directory behavior after its own reviewed approval and merge. The catalog amendment retains exact asset and attribution ownership. Checks and local review must identify the actual base/head under review.
...
Before executing the proposed lobby refactor, obtain actual approval of its exact reviewed amendment and confirm its governing documents have merged. The user acts as CEO; the planner acts as architect and owns the actionable epic plan. The orchestrator reads that plan, reconciles child issues, and maintains GitHub Project priorities, dependencies, ownership and status. The supplied catalog identities and placeholders are resolved, and D19's earlier scope is closed; the new [lobby refactor plan](lobby-refactor.md) owns only the changed room and UI behavior.
...
Continue evolving pre-flight with focused fast type/schema checks and deterministic short fixtures, keeping the entire command below 60 seconds. Ordinary pull-request CI enforces one five-minute wall-clock deadline across build, the relevant browser journey, the final gate and retries. Main CI has a ten-minute shared deadline from run start, including setup and queue time. A pull request labeled `release-gate-proof` runs the identical release matrix and ten-minute deadline before merge; adding or removing the label starts a fresh run, and this proof run's final `preflight` job is its acceptance gate. The three browser pairs run in parallel. Each pair must finish a measured 30-second direct gameplay interval with matching frames, state hashes, controller input, pause/resume, and no fatal page errors. Each pair also proves eight seconds of forced-relay play and its visible relay state. The final matrix check requires all six current-candidate records and matching source and artifacts. The [fast main delivery plan](fast-main-delivery.md) owns automatic deployment and separate post-release qualification.

Presubmit coverage must include fast deterministic checks for canonical serialization and malformed input, matching/mismatching fingerprints, atomic joins and authorization, privacy of outbound traffic, coordinated epoch changes, and local save compatibility, plus a real browser journey relevant to the change. The ten-minute network session matrix and full core suite no longer block main delivery. [Post-release qualification](../../.github/workflows/post-release-qualification.yml) runs daily at 03:17 UTC and through GitHub Actions → Post-release qualification → Run workflow on main. It builds one diagnostic fixture, then runs `spikes/d02/ci_job.sh core` and `network-full` for all three browser pairs in parallel under one 30-minute deadline, with no retry extension. Its summary reports each job, actual 600-second run times, matrix verification and failure artifacts. Investigate failures and roll back a confirmed live regression; infrastructure-only failures remain visible without undoing a healthy release. The release gate tests forced TURN and direct connection with credentials scoped to the run.
...
Essential pull-request evidence uses focused checks for the changed journey. The complete core/network matrix is owned by scheduled/manual post-release qualification. The 30-minute-per-route AC-16 sessions, full per-hardware qualification and repeated startup samples remain separately timed staging validation jobs with named developer/operator ownership. Report release-validation results separately from pull-request CI and post-submit qualification.
...
Each implementation PR records exact commit/build, relevant AC IDs, test settings, actual output and duration, inspected visuals where relevant, pre-flight result, CI URL and elapsed time, and any post-submit plan. Use a fresh local reviewer in a separate detached worktree with only the PR URL and review instructions. Publish its report per [the handoff contract](../../tooling/vaseline/docs/agent/review-handoff.md). Follow the scoped merge authorization recorded in the epic. The user now authorizes agent merges after local review and passing required checks, superseding the original external-review/separate-merger default for this project. Preserve branch protections and review and merge governing plans before implementation. Faithful requested behavior needs no second approval; ask before adding a new player-facing design the user did not request or reasonably imply. Confirm integration before dependent work. The original approved snapshot remains available at its recorded commit.

## Hosting and operational delivery

At P6 compare available HTTPS/static hosting, one small Node coordinator, and TURN service prices/quotas using current provider documentation. The $50 warning target/$100 forecast shutdown trigger is not a price quote or guaranteed cap. Build a cost worksheet from fixed service charges plus measured catalog egress, logs and TURN bytes at 20 concurrent sessions/100 directory connections, including voice and relay-only use. Model monthly session-hours explicitly; concurrent capacity is not a monthly usage estimate. Start beta at a lower admission limit if measured spend demands it and realign any change to the accepted capacity.

Provision only after meeting the alert, traffic-limit and shutdown gates in the [D24 AWS deployment plan](d24-aws-eb-staging.md). Configure relay credentials with short expiries, allocation/bandwidth quotas, directory/signaling limits, TLS/origin checks and log redaction. Exercise capacity exhaustion: new sessions receive a capacity explanation, relay-only users never silently disclose their address by falling back to direct, and the operator receives a content-free alert.
...
| Relay use exceeds budget | Follow the [D24 cost procedure](d24-aws-eb-staging.md); measure session-hours and verify quota and admission behavior before provisioning. |
...
| Storage/content | Local IndexedDB saves and verified guest ROM cache; authorized catalog assets with manifests/credits; temporary server copy of a hosted custom ROM while its room exists. The proposed Create Game plan adds local-host imports to the same browser library. | D03 content identity is integrated; [the catalog amendment](included-games.md) governs D19 packaging and presentation. |
| Hosting | Static HTTPS assets, one coordinator, STUN/TURN; reuse managed services where they meet enforceable quotas. | D24 owns the [AWS cost operating procedure](d24-aws-eb-staging.md); the user accepted residual charges above $100 for the public site. D22 retains broader beta capacity qualification. |
