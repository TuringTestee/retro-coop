...
Provision only after recording the user-approved monitored cost policy, a current 31-day worksheet, $50 actual and $100 forecast billing alerts, traffic limits and a tested shutdown runbook. Configure relay credentials with short expiries, allocation/bandwidth quotas, directory/signaling limits, TLS/origin checks and log redaction. Exercise budget exhaustion: new sessions receive a capacity explanation, relay-only users never silently disclose their address by falling back to direct, and the operator receives a content-free alert.
...
| Relay use exceeds budget | Current-price worksheet, metered session-hours, quota/admission tests, $50/$100 alerts and shutdown rehearsal before provisioning. |
...
| Hosting | Static HTTPS assets, one coordinator, STUN/TURN; reuse managed services where they meet enforceable quotas. | D24/D22 operator verifies current prices, $50 actual/$100 forecast alerts, quotas and shutdown before spend; the user accepted residual charges above $100 for the public AWS site. |
