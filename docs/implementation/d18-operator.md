...
The author pass reproduced a room label changing terminal text direction in CLI output. The [before failure](d18-operator/terminal-output/before.txt) and [after results](d18-operator/terminal-output/after.txt) exercise the actual CLI. Listing and confirmation output now escape terminal control/direction characters while leaving the stored label unchanged. Nine operator tests plus five proxy tests pass; this terminal-only repair does not change the previously tested browser or coordinator behavior.

## Independent review repair: fresh and retried admission

Round 1 identified that blocked initial upgrades returned HTTP 403, which browsers expose only as a generic WebSocket failure. Existing connected clients received close code 4003 correctly, but retries, new tabs and reloads lost the restriction reason. The [before browser failure](d18-operator/retry-repair/before-failure.txt) and reviewer wire reproduction establish this distinction. A separate detached `470c884` build confirms the [generic prior message](d18-operator/retry-repair/retry-before.png), with [observed text/assertion](d18-operator/retry-repair/visual-before.txt). To reproduce this old-head visual check, copy [the probe](d18-operator/retry-repair/before_probe.py) over `scripts/rooms/operator_smoke.py` in that detached build and run the normal browser command; its expected restriction assertion fails.

After validating the existing origin, path, connection and attempt limits, blocked upgrades now send close code 4003 without authenticating or admitting a session. Every attempt still consumes the existing bounded admission rate; other denials remain distinct. Room and directory feedback share the explicit restriction reason, and the existing reconnect action remains available. Successful post-expiry authentication clears the restriction and stale peer-disconnection copy. Old revoked tokens do not regain authority.

The committed runtime [candidate](d18-operator/retry-repair/candidate.json) passed the expanded actual [browser flow](d18-operator/retry-repair/browser.json) in **13.80 seconds**: connected removal/blocking and local-state preservation, directory retry, fresh tab, reload, actual block expiry, restored access and a new room. Inspected [restricted](d18-operator/retry-repair/retry.png) and [expired/recovered](d18-operator/retry-repair/expired.png) screenshots show distinct states. The initial visual check exposed stale peer-disconnection copy after successful recovery; the first neutral-status assertion then caught the shared close method's default closed message. The final candidate explicitly publishes the neutral state and passes the same assertion. The delivery-time focused probe and shared deadline were unchanged; current policy is owned by the governing [verification strategy](browser-nes-platform.md#verification-strategy). Current final preflight and independent reassessment are recorded on PR #63.

A matching-case author audit expanded the CLI regression to ALM, LRM and RLM alongside the original override character. The manually listed range missed those marks ([before](d18-operator/terminal-output/all-bidi-before.txt)). The output boundary now uses Unicode’s `Bidi_Control` property as the single category definition, and both actual listing and confirmation previews escape every tested mark ([after](d18-operator/terminal-output/all-bidi-after.txt)). This terminal-only correction leaves the recorded browser/coordinator behavior and build unchanged.
