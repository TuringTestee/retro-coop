...
- `packages/contracts/src/rooms.ts`: strict room-control union and validation. D16 extends this protocol with [bounded temporary text chat](d16-chat.md); the D08 evidence below records the earlier metadata-only slice. All commands require a request ID. Unknown fields, malformed fingerprints, control characters and binary frames are rejected. The server caps frames at 4 KiB and outgoing backlog at 64 KiB; there is no ROM/save/upload endpoint.
...
## Independent round 1 repairs

A recovered host now checks room identity before committing a validated replacement worker. Declining preserves the active local game and room; after reload it preserves the room and guest while leaving the player unloaded. Invalid and cancelled selections never replace either. Approval is scoped to the recovered room and exact fingerprint; the host close path also requires confirmation when no matching approval exists. Fresh tabs do not wait for the coordinator before local play. Existing-session recovery is bounded by the connection deadline; offline local play remains available, and later room replacement still requires confirmation.

`packages/contracts/src/fingerprint.ts` is the canonical owner of Fingerprint, Cartridge, local schema/settings constants, strict validation and exact identity comparison. Player construction, cartridge inspection and room contracts consume or reexport these definitions. The validator still rejects unknown metadata, malformed hashes and incompatible settings. Independent test literals keep expected identities separate from implementation constants. The neighboring audit found and removed the duplicated cartridge shape as well as the reported full fingerprint shape.

Current repair evidence: `d08/repair-preflight.txt` (20 Node tests, full preflight 7.22 seconds), `d08/repair-browser.json` (7.25 seconds), and `d08/repair-foundation.json` (11.72 seconds). The real browser test exercises active-game decline, reload/different-file decline, cancelled read, invalid file and confirmed replacement with a live guest. Inspected `d08/replacement-declined.png` and `d08/replacement-confirmed.png` show retained versus replaced rooms. Earlier integrated evidence is historical; this repair pass is author verification and awaits independent reassessment.
