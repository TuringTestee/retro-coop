...
| UJS-7 | A second browser saw a waiting Super Tilt Bro room with game, host, occupancy, code, and Join, then reached synchronized shared play with one Join click and no picker. A public arbitrary-ROM room showed the exact-local-file action and reached shared play only after a matching local selection; captured WebSocket frames contained neither private filename nor ROM bytes. An unlisted room left the public directory IDs unchanged, appeared through its copied invitation, and completed the same local-match path. |
...
[UJS-7 machine-readable results](d19-catalog/browser/public-built-in-join.json) cover all three friend-entry variants against the real client and coordinator. The built-in path records the exact Super Tilt fingerprint, one directory Join click, no file chooser, matching frame-360 state hashes, 705×661 canvases, and exact 1280×800 document bounds. Inspectable captures show the [public built-in directory row](d19-catalog/browser/public-built-in-directory.png), [resulting shared play](d19-catalog/browser/public-built-in-playing.png), [public arbitrary-ROM match action](d19-catalog/browser/public-arbitrary-match-required.png), and [unlisted invitation entry](d19-catalog/browser/unlisted-invitation-entry.png). The arbitrary pending room and unlisted invitation panels both have equal scroll/client heights, so their actions are not hidden behind internal scrolling.
