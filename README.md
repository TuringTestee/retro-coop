...
After a merge, main CI plays the same game for 30 measured seconds in each of three browser pairs: Chrome–Chrome, Firefox–Firefox, and Chrome–Firefox. It checks shared frames, hashes, input, pause/resume, errors, and direct connection, then exercises forced relay. The three pairs run in parallel within a ten-minute CI deadline. Add the `release-gate-proof` label to a pull request when its release gate needs proof before merge; that PR runs the same matrix within the same ten-minute deadline. Ordinary PRs keep the five-minute gate. The longer core and network qualification is kept for the separate post-release workflow; see the [verification strategy](docs/implementation/browser-nes-platform.md#verification-strategy).
