...
type Core = WebAssembly.Exports & {
 ...
 local_state_limit():number; local_state_alloc(size:number):number; local_state_export():number; local_state_import(ptr:number,size:number):number;
...
onmessage = async ({data}: MessageEvent<unknown>) => {
 try {
  ...
  if (data.type === 'load') {
   ...
   if (data.type === 'battery-export' || data.type === 'state-export') {
    ...
    check(api.import(ptr,data.bytes.byteLength));
    send({type:`${kind}-imported`,requestId:data.requestId});
   }
  }
