...
function App() {
 ...
 const load = (file?:File,selectionCurrent?:()=>boolean) => {if(file && (selectionCurrent || rooms.current?.beforeSelection() !== false)) void runtime.current?.load(file,(fingerprint,isCurrent)=>rooms.current?.approveSelection(fingerprint,isCurrent,file) ?? Promise.resolve(isCurrent()),true,selectionCurrent);};
 ...
 return <main className={`${showDiscovery?'discovery':showRoom?'waiting-room':'playing'}${browsing?' browsing':''}${inviting?' inviting':''}${state.shared?' shared-session':''}${sessionOpen?' session-open':''}`} data-coordinator={clientConfig.coordinatorUrl}>
  <header><a href="/" className="brand">RETRO COOP</a><span data-testid="guest">{guest}</span>{!inviting&&(roomView||state.loaded)&&<button onClick={()=>{setBrowsing(value=>!value);setSessionOpen(false);}}>{browsing?roomView?'Return to room':'Return to game':'Public rooms'}</button>}{!browsing&&roomView?.started&&<button aria-expanded={sessionOpen} aria-controls="room-session" onClick={()=>{setSessionOpen(value=>!value);if(sessionOpen)canvas.current?.focus();}}>{sessionOpen?'Close room':'Room'}</button>}<button onClick={()=>setSettings(true)}>Settings</button></header>
