...
function App() {
 ...
 return <main className={`${showDiscovery?'discovery':'playing'}${browsing?' browsing':''}${state.shared?' shared-session':''}${sessionOpen?' session-open':''}`} data-coordinator={clientConfig.coordinatorUrl}>
  <header><a href="/" className="brand">RETRO COOP</a><span data-testid="guest">{guest}</span>{state.loaded&&<><button onClick={()=>{setBrowsing(value=>!value);setSessionOpen(false);}}>{browsing?'Return to game':'All lobbies'}</button>{!browsing&&<button aria-expanded={sessionOpen} aria-controls="room-session" onClick={()=>setSessionOpen(value=>!value)}>{sessionOpen?'Close details':roomStartRequired?'Start game':'Room'}</button>}<button onClick={()=>setSettings(true)}>Settings</button></>}</header>
