...
function App() {
 ...
 return <main data-coordinator={clientConfig.coordinatorUrl}>
  ...
  <section className={`panel ${drag ? 'drag' : ''}`} aria-labelledby="player-title" onDragOver={event => {event.preventDefault(); setDrag(true);}} onDragLeave={event => {if(!event.currentTarget.contains(event.relatedTarget as Node)) setDrag(false);}} onDrop={event => {event.preventDefault();setDrag(false);if(event.dataTransfer.files.length === 1) load(event.dataTransfer.files[0]); else runtime.current?.rejectSelection('Choose one NES cartridge at a time. Your previous game is preserved.');}}>
   <div><p className="eyebrow">Local play · Player 1</p><h2 id="player-title">{state.loaded ? 'Your local game' : 'Drop your NES game here'}</h2>
   ...
   <p role="status" aria-live="polite" data-testid="player-status">{state.status}</p>{state.audioIssue && <p className="hint">{state.audioIssue} <button onClick={() => runtime.current?.retryAudio()}>Retry sound</button></p>}
  ...
  </section><footer>Your game runs in this browser. Only room names and compatibility metadata go to the room service; your file stays here.</footer>
 </main>;
}
