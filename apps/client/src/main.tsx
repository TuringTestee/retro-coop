...
function App() {
 ...
 const [browsing,setBrowsing]=useState(false),[sessionOpen,setSessionOpen]=useState(false);
 const [roomStartRequired,setRoomStartRequired]=useState(false);
 ...
 return <main className={`${showDiscovery?'discovery':'playing'}${browsing?' browsing':''}${state.shared?' shared-session':''}${sessionOpen?' session-open':''}`} data-coordinator={clientConfig.coordinatorUrl}>
  <header><a href="/" className="brand">RETRO COOP</a><span data-testid="guest">{guest}</span>{state.loaded&&<><button onClick={()=>{setBrowsing(value=>!value);setSessionOpen(false);}}>{browsing?'Return to game':'All lobbies'}</button>{!browsing&&<button aria-expanded={sessionOpen} aria-controls="room-session" onClick={()=>{setSessionOpen(value=>!value);if(sessionOpen)canvas.current?.focus();}}>{sessionOpen?'Close details':roomStartRequired?'Start game':'Room'}</button>}<button onClick={()=>setSettings(true)}>Settings</button></>}</header>
  <input ref={picker} type="file" accept=".nes" hidden aria-label="NES cartridge file" onChange={event => load(event.target.files?.[0])}/>
  <RoomPanel showDiscovery={showDiscovery} onChoose={choose} onBrowse={()=>setBrowsing(true)} onIncluded={load} selectionLoading={state.loading} ref={rooms} controls={controls} onVoice={setVoice} player={()=>runtime.current} fingerprint={state.fingerprint} onNickname={setGuest} policy={policy} changePolicy={changePolicy} onConnection={setConnection} onRoomStartRequired={setRoomStartRequired} onHost={()=>{panel.current?.querySelector('button')?.focus();}}/>
