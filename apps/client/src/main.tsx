...
function App() {
 ...
 return <main className={`${showDiscovery?'discovery':'playing'}${browsing?' browsing':''}${state.shared?' shared-session':''}${sessionOpen?' session-open':''}`} data-coordinator={clientConfig.coordinatorUrl}>
  ...
  <RoomPanel showDiscovery={showDiscovery} onChoose={choose} onIncluded={load} selectionLoading={state.loading} ref={rooms} controls={controls} onVoice={setVoice} player={()=>runtime.current} fingerprint={state.fingerprint} onNickname={setGuest} policy={policy} changePolicy={changePolicy} onConnection={setConnection} onRoomStartRequired={setRoomStartRequired} onHost={()=>{panel.current?.querySelector('button')?.focus();}}/>
