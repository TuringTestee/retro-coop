...
export function ConnectionPolicyControl({policy,change,compact=false}:{policy:ConnectionPolicy;change:(policy:ConnectionPolicy)=>void;compact?:boolean}) {
 const id=useId();
 ...
 <p className="hint">{compact?'Standard may reveal your network address to the other player. Relay only depends on relay capacity.':'Standard may reveal your network address to the other player. Relay only uses an encrypted relay and depends on capacity. The service and relay operator still see connection metadata. If either player selects Relay only, both use it before exchanging peer candidates.'}</p></div>;
}
