...
export const RoomPanel = forwardRef<RoomPanelHandle,{playCards?:React.ReactNode;renderFps?:number;showDiscovery:boolean;releaseInFullscreen:boolean;onChoose():void;onCreate():void;onBrowse():void;onInvitationDismiss():void;onAcquired:(file:File,current:()=>boolean)=>boolean;selectionLoading:boolean;controls:Controls;onVoice:(state:VoiceState|undefined)=>void;fingerprint?:Fingerprint;player:()=>LocalPlayer|null;onNickname:(name:string)=>void;policy:ConnectionPolicy;changePolicy:(policy:ConnectionPolicy)=>void;onConnection:(status:string)=>void;onRoomChange:(room?:RoomView)=>void;onState:(state:RoomState)=>void}>(function RoomPanel({playCards,renderFps,showDiscovery,releaseInFullscreen,onChoose,onCreate,onBrowse,onInvitationDismiss,onAcquired,selectionLoading,controls,onVoice,fingerprint,player,onNickname,policy,changePolicy,onConnection,onRoomChange,onState},ref) {
 ...
 const cancelLeave=()=>{leaveFocusPending.current=true;setConfirmLeave(false);setLeaveError('');};
 ...
 const directShared=room?.started==='shared'&&state.connected&&room.peer.status==='connected'&&state.connection?.route==='direct';
 const connectionNode=!directShared&&(room?.peer.epoch||!state.connected)?<p role="status" className={room?.started==='shared'&&state.connection?.route==='relay'&&room.peer.status==='connected'?'relay-notice':undefined} data-testid="connection-status">{connectionStatus(state)}</p>:null;
 const leaveControl=room&&!(room.role==='guest'&&!room.catalogId&&['checking','downloading','loading','expired'].includes(guestAcquisition?.phase??''))&&(confirmLeave?<div className="room-confirm" role="group" aria-label="Confirm leave"><p>{room.role==='host'?'Close this room for both players? Your local game stays available.':'Leave shared play? Your local game stays available.'}</p><button disabled={state.busy} onClick={()=>void leaveRoom(room)}>Confirm leave</button><button onClick={cancelLeave}>Stay in room</button>{leaveError&&<p role="alert">{leaveError}</p>}</div>:<button data-leave-room onClick={()=>{if(room.role==='host'||room.established){setConfirmLeave(true);requestAnimationFrame(()=>document.querySelector<HTMLButtonElement>('.room-confirm button')?.focus());}else void leaveRoom(room);}}>Leave room</button>);
