...
export const RoomPanel = forwardRef<RoomPanelHandle,{playCards?:React.ReactNode;showDiscovery:boolean;releaseInFullscreen:boolean;onChoose():void;onCreate():void;onBrowse():void;onInvitationDismiss():void;onAcquired:(file:File,current:()=>boolean)=>boolean;selectionLoading:boolean;controls:Controls;onVoice:(state:VoiceState|undefined)=>void;fingerprint?:Fingerprint;player:()=>LocalPlayer|null;onNickname:(name:string)=>void;policy:ConnectionPolicy;changePolicy:(policy:ConnectionPolicy)=>void;onConnection:(status:string)=>void;onRoomChange:(room?:RoomView)=>void;onState:(state:RoomState)=>void}>(function RoomPanel({playCards,showDiscovery,releaseInFullscreen,onChoose,onCreate,onBrowse,onInvitationDismiss,onAcquired,selectionLoading,controls,onVoice,fingerprint,player,onNickname,policy,changePolicy,onConnection,onRoomChange,onState},ref) {
 ...
 const [placeRetry,setPlaceRetry]=useState<PlaceAttempt>();
 ...
 const leaveRoom=async(room:RoomView)=>{cancelIncluded('');setIncludedStatus('');const left=room.role==='host'?await client.current?.act({type:'close',roomId:room.id})??false:(cancelGuest(),await client.current?.act({type:'leave',intent:room.reservationIntent!})??false);if(!left){if(room.catalogId)setIncludedStatus('Room remains open. Retry the included game if Start is unavailable.');setLeaveError('Could not leave the room. Retry or stay here.');return;}setConfirmLeave(false);setLeaveError('');const nextInvite=new URLSearchParams(location.hash.slice(1)).get('invite');if(nextInvite&&nextInvite!==invite){setInvite(nextInvite);onBrowse();}else if(invite)clearInvitation();else onBrowse();requestAnimationFrame(()=>document.querySelector<HTMLInputElement>('.directory-panel input')?.focus());};
 const cancelLeave=()=>{setConfirmLeave(false);setLeaveError('');requestAnimationFrame(()=>document.querySelector<HTMLButtonElement>('[data-leave-room]')?.focus());};
 ...
 useImperativeHandle(ref,()=>({voice:()=>client.current?.voice,localPlayIntent(){client.current?.localPlayIntent();},readyToResume(){client.current?.readyToResume();},recoverRelease,isGuest(){return client.current?.isGuest()??false;},
  beforeSelection() {
 ...
 const changeGuestPlace=async(attempt:PlaceAttempt)=>{const current=guestRoom.current;if(!current||current.id!==attempt.roomId||current.role!=='host'||current.started)return;setSlotPending(true);setSlotError(false);setPlaceRetry(attempt);const ok=await client.current?.act({type:'guestPlace',...attempt});const latest=guestRoom.current,applied=latest?.id===attempt.roomId&&latest.guestPlace===attempt.place&&latest.guestPlaceVersion>attempt.expectedVersion;setSlotPending(false);setSlotError(!ok&&!applied);if(ok||applied)setPlaceRetry(undefined);};
 ...
 const connectionNode=(room?.peer.epoch||!state.connected)?<p role="status" data-testid="connection-status">{connectionStatus(state)}</p>:null;
 ...
 {(room||invite)&& <section id="room-session" className={`room-panel${invite&&!room?' invitation':''}${room&&!fingerprint?' pending-room':''}`} aria-labelledby="room-heading">
  ...
  {room?.started==='shared'&&(confirmLeave?<div className="room-confirm" role="group" aria-label="Confirm leave"><p>{room.role==='host'?'Close this room for both players? Your local game stays available.':'Leave shared play? Your local game stays available.'}</p><button disabled={state.busy} onClick={()=>void leaveRoom(room)}>Confirm leave</button><button onClick={cancelLeave}>Stay in room</button>{leaveError&&<p role="alert">{leaveError}</p>}</div>:<button data-leave-room onClick={()=>{setConfirmLeave(true);requestAnimationFrame(()=>document.querySelector<HTMLButtonElement>('.room-confirm button')?.focus());}}>Leave room</button>)}
  {playCards}
  ...
  {room?.role==='guest'&&!room.catalogId&&guestAcquisition&&(!room.started||guestAcquisition.phase!=='loaded')&&<div className="guest-acquisition" role="status" aria-live="polite"><p>{guestAcquisition.message}</p>{guestAcquisition.notice&&<p>{guestAcquisition.notice}</p>}{['checking','downloading','loading'].includes(guestAcquisition.phase)&&<button onClick={()=>void leaveGuest()}>Cancel preparation</button>}{guestAcquisition.phase==='failed'&&<button onClick={()=>void startGuest(room)}>Retry download</button>}{guestAcquisition.phase==='expired'&&<button onClick={()=>void leaveGuest()}>Return to rooms</button>}</div>}
  ...
  {room&&room.started!=='shared'&&!(room.role==='guest'&&!room.catalogId&&['checking','downloading','loading','expired'].includes(guestAcquisition?.phase??''))&&(confirmLeave?<div className="room-confirm" role="group" aria-label="Confirm leave"><p>{room.role==='host'?'Close this room for both players? Your local game stays available.':'Leave shared play? Your local game stays available.'}</p><button disabled={state.busy} onClick={()=>void leaveRoom(room)}>Confirm leave</button><button onClick={cancelLeave}>Stay in room</button>{leaveError&&<p role="alert">{leaveError}</p>}</div>:<button data-leave-room onClick={()=>{if(room.role==='host'||room.established){setConfirmLeave(true);requestAnimationFrame(()=>document.querySelector<HTMLButtonElement>('.room-confirm button')?.focus());}else void leaveRoom(room);}}>Leave room</button>)}
