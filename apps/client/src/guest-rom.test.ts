...
import {clearLocalData,putRom,readRom} from './saves.ts';
...
test('guest download checks exact bytes, persists, reads back and rehashes on reuse',async()=>{
 ...
 const stored=await readRom(hash);const damaged=bytes.slice();damaged[20]^=1;await putRom({sha256:hash,size:damaged.length,bytes:damaged.buffer,savedAt:Date.now()},stored.generation);
