...
export function ControllerMode({room,act}:{room:RoomView;act:RoomClient['act']}) {
 ...
 const name=(role:RoomRole)=>role==='host'?`${room.host} (host)`:`${room.guest??'Guest'} (guest)`;
 ...
 return <section className="controller-mode" aria-labelledby="controller-heading" data-testid="controller-mode"><h3 id="controller-heading">Session controllers</h3>
  ...
  <p data-testid="controller-ownership">{assignment.mode==='shared'?'Shared P1':'Separate P1/P2'} · P1: {name(assignment.p1)}{assignment.mode==='separate'?` · P2: ${name(assignment.p1==='host'?'guest':'host')}`:' · P2: no input'}</p>
