...
type Room = { game:GameSession; started?:'solo'|'shared'; established?:boolean; guestReconnectUntil?:number; chat:RoomChat; id:string; invite:string; code?:string; label:string; visibility:Visibility; host:Session; guest?:Session; reservationUntil?:number; guestIntent?:string; guestMembership?:string; guestFile?:Fingerprint; fingerprint:Fingerprint; intent:string; confirmed:boolean; created:number; reconnectUntil?:number; kicked:Set<string> };
 ...
 handle(token:string,command:Exclude<RoomCommand,{type:'hello'}>,sender?:Sender): RoomData {
  ...
  switch(command.type) {
   ...
   case 'startRoom': {
    ...
    if(readyGuest){try {room.game.requestStart();}catch(error){throw new RoomError(error instanceof Error?error.message:'game_prerequisites');}room.started='shared';this.publish(room);}
