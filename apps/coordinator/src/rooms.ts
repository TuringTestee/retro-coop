...
export class Rooms {
 ...
 handle(token:string,command:Exclude<RoomCommand,{type:'hello'}>,sender?:Sender): RoomData {
  ...
  switch(command.type) {
   ...
   case 'guestAcquisition': {const room=this.room(session);if(room.id!==command.roomId||room.guest!==session||room.guestMembership!==command.membership)throw new RoomError('membership_changed');if(room.guestAcquisition!==command.phase){room.guestAcquisition=command.phase;this.publish(room,false);}return {room:this.view(room,session)};}
  }
  return {};
 }
 sweep() {
